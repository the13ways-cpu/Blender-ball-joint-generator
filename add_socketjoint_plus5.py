bl_info = {
    "name": "Ball Joint Generator (synced offsets + segments)",
    "author": "ChatGPT",
    "version": (1, 2, 0),
    "blender": (3, 0, 0),
    "location": "View3D > Add > Mesh",
    "description": "Generate male ball joint (visible) and synced socket cutter (boolean preview) with adjustable segments.",
    "category": "Object",
}
...
class OBJECT_OT_add_socket_and_male(bpy.types.Operator):
    ...
    # --- 描画軽量化のためのセグメント可変 ---
    ball_segments: bpy.props.IntProperty(
        name="Ball Segments", default=32, min=4, max=128
    )
    ball_rings: bpy.props.IntProperty(
        name="Ball Rings", default=16, min=4, max=64
    )
    cylinder_vertices: bpy.props.IntProperty(
        name="Cylinder Vertices", default=32, min=3, max=64
    )
    cone_vertices: bpy.props.IntProperty(
        name="Cone Vertices", default=32, min=3, max=64
    )
    ...
    def _create_ball(self, shape, radius, origin, rotation):
        if shape == 'SPHERE':
            bpy.ops.mesh.primitive_uv_sphere_add(
                ...
                location=origin,
                rotation=rotation,
                segments=self.ball_segments,
                ring_count=self.ball_rings
            )
        elif shape == 'CYLINDER':
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=origin,
                rotation=rotation,
                vertices=self.cylinder_vertices
            )
        elif shape == 'CUBE':
